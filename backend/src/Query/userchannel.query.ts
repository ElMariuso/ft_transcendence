...
export class UserChannelQuery
{
	...
	{
		const userchannel = await this.prisma.user_Channel.findUnique
		(
			{
	 ...
	 * @returns  Updated record
	 * 
	 * @query update user_channel set user_channel.idStatus = $idStatus where user_channel.idUser_Channel = $idUserChannel;
	 */
	async acceptChannel(idUserChannel: number)
	{
		const userchannel = await this.prisma.user_Channel.update
		(
			{
	 ...
	 * @query update user_channel set user_channel.idStatus = $idStatus where user_channel.idUser_Channel = $idUserChannel;
	 */
	async refuseChannel(idUserChannel: number)
	{
		const userchannel = await this.prisma.user_Channel.update
		(
			{
